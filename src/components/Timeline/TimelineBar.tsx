...
export const TimelineBar: React.FC<TimelineBarProps> = ({
  ...
  return (
    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-30 w-[95%] max-w-5xl">
      <div className="glass-panel p-4 flex flex-col gap-3 border border-white/10 shadow-2xl backdrop-blur-2xl">
        {/* Top bar: Date frame info + Quick Date Picker Trigger */}
        <div className="flex items-center justify-between text-xs font-mono">
          <div className="flex items-center gap-2">
            <button
              onClick={onOpenDateFramePicker}
              className="glass-button text-cyan-300 hover:text-cyan-200 text-xs px-3 py-1.5 rounded-lg border border-cyan-500/30 flex items-center gap-2"
            >
              <Calendar className="w-3.5 h-3.5 text-cyan-400" />
              <span>Frame: {formattedStart} ➔ {formattedEnd}</span>
            </button>
          ...
          <div className="flex items-center gap-3">
            {data.trips.length > 0 && (
              <div className="flex items-center gap-1.5 text-slate-300">
                ...
                <select
                  ...
                  className="bg-slate-900/80 border border-white/15 text-white text-xs rounded-lg px-2 py-1 outline-none cursor-pointer max-w-[200px] truncate"
                  onChange={(e) => {
                  ...
                  defaultValue=""
                >
                  <option value="" disabled>Select Journey / Trip...</option>
            ...
            <div className="text-cyan-400 font-bold bg-cyan-500/10 px-2.5 py-1 rounded-md border border-cyan-500/20">
              {formattedCurrent}
            </div>
          </div>
        </div>
        ...
        <div className="relative flex flex-col gap-1">
          ...
          <div
            className="absolute top-0 left-0 h-[6px] rounded-full bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-500 pointer-events-none opacity-80"
        ...
        <div className="flex items-center justify-between pt-1">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="glass-button text-slate-400 hover:text-white p-2 rounded-lg"
            >
            ...
            <button
              ...
              title="Step Backward"
              className="glass-button text-slate-300 hover:text-white p-2 rounded-lg"
            >
            ...
            <button
              ...
              className={`glass-button text-xs px-2.5 py-1.5 rounded-lg border ${
                loop ? 'border-cyan-400 text-cyan-300 bg-cyan-500/10' : 'text-slate-400 border-white/10'
              }`}
            >
              Loop: {loop ? 'ON' : 'OFF'}
            </button>
